...
const initDatabase = async () => {
  try {
    await pool.query(`
      ...
      -- Ensure default exists if table was created earlier
...
app.get(['/api/user/survey/history', '/api/usuario/historial/encuesta'], authenticateToken, async (req: any, res) => {
  res.json([]); // Placeholder for history
});
...
app.get('/api/stats', async (req, res) => {
  ...
  res.json({ totalUsers: parseInt(u.rows[0].count), completedSurveys: parseInt(s.rows[0].count), pendingSurveys: 0, registeredEvents: 0, educationDist: [] });
});
